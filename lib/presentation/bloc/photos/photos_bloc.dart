...
//       try {
//         final images = await pexelsApiService.fetchClothingModelImages(event.page); // Now returns List<ImageModel>
//         if (images.isEmpty) {
//           emit(ImageLoaded(state is ImageLoaded ? (state as ImageLoaded).images : [], true));
//         } else {
//           emit(
//             ImageLoaded(
//               state is ImageLoaded ? (state as ImageLoaded).images + images : images, // Use List<ImageModel> here
//               false,
//             ),
//           );
//           page++;
  ...
  ImageBloc(this.pexelsApiService) : super(ImageLoading()) {
    on<LoadImages>((event, emit) async {
      ...
      try {
        ...
        if (images.isEmpty) {
          // No more images to load, set hasReachedMax to true
          emit(ImageLoaded(state is ImageLoaded ? (state as ImageLoaded).images : [], true));
        } else {
          emit(
            ImageLoaded(
              state is ImageLoaded ? (state as ImageLoaded).images + images : images,
              false, // Set hasReachedMax to false as we are still loading images
            ),
          );
          page++;  // Increase the page number after a successful fetch
        }
      } catch (error) {
        emit(ImageError());
      } finally {
